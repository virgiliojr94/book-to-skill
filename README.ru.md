...
> **Книга, живущая в вебе — или в лекционном видео/записи — не доходит до `/book-to-skill`.** Источник сначала должен стать локальным файлом/папкой/glob с текстовым слоем. Книга только на веб-странице, видео/запись доклада или скан (если не хотите OCR вручную) не получится указать напрямую. Сначала прогоните их через [cue-omni-reader](https://github.com/sensedeal/cue-skills/tree/main/cue-omni-reader), чтобы получить Markdown — веб-страницы (включая видео/вложения на странице) + авторизованные локальные документы/аудио/видео, сразу несколько локальных файлов — затем отдайте полученный текст в `/book-to-skill`. Установка одной строкой: `npx skills add sensedeal/cue-skills --skill cue-omni-reader` (MIT; может тарифицироваться).
