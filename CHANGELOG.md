...
### Changed
- PDF cleanup (dehyphenation + repeated header/footer/page-number removal) now
  also runs on the `pypdf` and `pdfminer` extraction paths, not just `pdftotext`,
  so PDF text is cleaned consistently regardless of which extractor is used.
