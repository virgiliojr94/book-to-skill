...
Cross-agent notes (informational; ignored by host agents):
  - Compatible skill roots: GitHub Copilot CLI (~/.copilot/skills, ~/.agents/skills,
    ...
    OpenClaw (~/.openclaw/skills, ~/.agents/skills, .agents/skills, skills/).
...
2. Cross-agent personal skills (Copilot, Amp, Codex, OpenClaw): `~/.agents/skills/`
...
11. OpenClaw personal skills: `~/.openclaw/skills/` or `~/.agents/skills/`
12. OpenClaw project skills: `.agents/skills/` or `skills/` or `~/.openclaw/skills/`
...
CANDIDATES=(
  ...
  "$HOME/.openclaw/skills/book-to-skill/scripts/extract.py"
...
For OpenClaw, `~/.agents/skills` is shared with other cross-agent hosts and works out of the box. Use `~/.openclaw/skills` when the user prefers an OpenClaw-managed location. Verify discovery with `openclaw skills list` after generation.
...
4. If you cannot identify the host, ask: "Which agent are you running this in — OpenClaw, Hermes Agent, GitHub Copilot CLI, Amp, Codex, or Claude Code?"
